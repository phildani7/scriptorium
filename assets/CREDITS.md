...
library (`/public/cliparts`) are stock and generated assets licensed to the
project creator. The licence covers use in produced video and **requires no
attribution**.

All were normalized at import with ffmpeg. Image backgrounds are 1080×1920
crops; cliparts are downscaled to a 512 px bounding box with alpha preserved.

**Video loops — 34 in total.** Two are ~60 s at 1080×1920. The other 32 were
imported by `scripts/import-video-backgrounds.mjs`, which does three things to
each 5-second 720×1280 source:

- **strips the audio.** These play behind narration; a stray track would
  either fight the voice or make the offline renderer wait on a decode nobody
  listens to.
- **closes the loop.** A short runs 40–60 s, so a 5-second clip repeats ten
  times or more and any discontinuity becomes a visible tick on a schedule.
  The last 0.8 s is cross-faded over the first 0.8 s, which makes the final
  frame and the first frame the same picture. A palindrome would also be
  seamless but doubles the file and makes anything with direction — falling
  leaves, drifting smoke — run backwards half the time.
- **keeps the source resolution.** 720×1280 into a 1080×1920 frame: upscaling
  at import would add roughly twice the bytes and no detail, and these sit
  behind a palette-tinted scrim.

The set went from 164 MB of sources to 19 MB, posters included.

## Picker thumbnails (`/public/backgrounds/thumbs`)

Generated by `scripts/make-bg-thumbs.ts` from the assets themselves — CSS
treatments are screenshotted from the real template, images downscaled from
